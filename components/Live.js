...
export default class Live extends Component {
  ...
  askforPermission = () => {
    Permissions.askAsync(Permissions.LOCATION)
      .then(({ status }) => {
        //debugger;
        console.log("askforPermission status: ", status);
        if (status === 'granted') {
  ...
  setLocation = () => {
    Location.watchPositionAsync({
      ...
      console.log("setLocation coords: ", coords);
    ...
    if (status === 'undetermined') {
      return (
        <View style={styles.center}>
          <Foundation name='alert' size={50} />
          <Text>
            You should enable location services for this App.
          </Text>
          <TouchableOpacity style={styles.button} onPress={this.askforPermission}>
